...
.
├── GICGeometryPuzzle
│   ├── GICGeometryPuzzleApp.py # application code
├── GICGeometryPuzzleTest
│   ├── GICGeometryPuzzleTest.py # testcase code
├── start.sh # sample command to run the application
└── test.sh # sample test command
```

## Code Structure

```python
class ShapeChecker # utility shape checker static functions
class GeometryShape # shape class itself
class ApplicationConstants # contants like mode, string literals
class ApplicationContext # context when the application runs
class ApplicationRunner # execute the application
```

>The application runs exactly as the instruction gives. I used python as it is easier to run and simple

## Actual Code

Can use the online python interpreter to run the actual code from `GICGeometryPuzzleApp.py`, but the since the runtime is not exactly the same there are little minimal difference such as the line breaks `\n` are rendered differently.
